    ...
    uint64_t gmGap, uint32_t ubGap, uint32_t ubPad)
{
    ...
    constexpr uint32_t ELEMENT_BYTES = sizeof(typename TileData::DType);
    uint64_t dstStride =
        ((uint64_t(lenBurst) + ubPad * ELEMENT_BYTES + BLOCK_BYTE_SIZE - 1) / BLOCK_BYTE_SIZE + ubGap) *
        BLOCK_BYTE_SIZE / ELEMENT_BYTES;
    uint64_t srcStride = (lenBurst + gmGap) / ELEMENT_BYTES;
    ...
    __cbuf__ typename TileData::DType* dst, typename GlobalData::DType* src, uint32_t elements)
{
    ...
    uint64_t remainingBytes = uint64_t(elements) * sizeof(typename TileData::DType);
    // Copy complete 32-byte blocks without narrowing element counts or converting b64 lengths.
    while (remainingBytes >= BLOCK_BYTE_SIZE) {
        uint64_t chunkBlocks = remainingBytes / BLOCK_BYTE_SIZE;
        if (chunkBlocks > UINT16_MAX) {
            chunkBlocks = UINT16_MAX;
        }
        pto_copy_gm_to_cbuf(dstBytes, srcBytes, 0, 1, static_cast<uint16_t>(chunkBlocks), 0, 0);
        uint64_t chunkBytes = chunkBlocks * BLOCK_BYTE_SIZE;
        dstBytes += chunkBytes;
        srcBytes += chunkBytes;
        remainingBytes -= chunkBytes;
    }
    if (remainingBytes != 0) {
        // Pass the valid tail byte count; the conversion zero-pads the rest of the C0 block.
        pto_copy_gm_to_cbuf_multi_nd2nz(
            dstBytes, srcBytes, 0, 1, 1, static_cast<uint16_t>(remainingBytes), 0, 0, 1, 1, 0);
    }
}
