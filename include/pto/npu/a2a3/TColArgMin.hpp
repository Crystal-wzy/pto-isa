...
PTO_INTERNAL void TColArgMinCheck(
    unsigned srcValidRow, unsigned srcValidCol, unsigned dstValidRow, unsigned dstValidCol)
{
    static_assert(
        TileDataIn::ValidCol == 1 || TileDataIn::ValidCol == -1, "Fix: TCOLARGMIN Src ValidCol must be 1 or -1");
    static_assert(
    ...
    PTO_ASSERT(
        srcValidRow != 0 && srcValidCol != 0, "Fix: TCOLARGMIN input shape is invalid, validCol or validRow is 0.");
    PTO_ASSERT(dstValidRow != 1, "Fix: TCOLARGMIN output validRow must be 1");
    PTO_ASSERT(
        srcValidCol != dstValidCol, "Fix: TCOLARGMIN input validCol must be consistent with the output validCol");
}

template <typename TileDataOut, typename TileDataIn, typename TileDataTmp>
__tf__ PTO_INTERNAL void TColArgMin16(
    typename TileDataOut::TileDType __out__ dst, typename TileDataIn::TileDType __in__ src,
    typename TileDataTmp::TileDType __in__ tmp, unsigned srcValidRow, unsigned srcValidCol)
{
    ...
    if (numLoop > 0) {
        for (uint16_t j = 0; j < numLoop; j++) {
            ...
            for (uint16_t i = 1; i < srcValidRow; i++) {
                ...
                vcmp_le(
                    (__ubuf__ half*)tmpPtr + tmpGapEles, (__ubuf__ half*)srcPtr + i * srcRowStride + j * elemPerRpt, 1,
                    1, 1, 1, 0, 0, 0);
                pipe_barrier(PIPE_V);
                vsel(
                    (__ubuf__ half*)tmpPtr + 2 * tmpGapEles, (__ubuf__ half*)tmpPtr + 2 * tmpGapEles,
                    (__ubuf__ half*)tmpPtr, 1, 1, 1, 1, 0, 0, 0, 0);
                vmin(
                    (__ubuf__ half*)tmpPtr + tmpGapEles, (__ubuf__ half*)tmpPtr + tmpGapEles,
                    (__ubuf__ half*)srcPtr + i * srcRowStride + j * elemPerRpt, 1, 1, 1, 1, 0, 0, 0);
            }
            pipe_barrier(PIPE_V);

            vconv_s162f16a(
                (__ubuf__ half*)tmpPtr + 2 * tmpGapEles, (__ubuf__ int16_t*)tmpPtr + 2 * tmpGapEles, 1, 1, 1, 0, 0);
            pipe_barrier(PIPE_V);
            vconv_f162s32a(
                (__ubuf__ int32_t*)dstPtr + j * elemPerRpt, (__ubuf__ half*)tmpPtr + 2 * tmpGapEles, 2, 1, 1, 8, 4);
        }
    }
    if (remainAfterLoop > 0) {
        ...
        for (uint16_t i = 1; i < srcValidRow; i++) {
            ...
            vadds((__ubuf__ int16_t*)tmpPtr, (__ubuf__ int16_t*)tmpPtr, 1, 1, 1, 1, 0, 0);
            vcmp_le(
                (__ubuf__ half*)tmpPtr + tmpGapEles, (__ubuf__ half*)srcPtr + i * srcRowStride + numLoop * elemPerRpt,
                1, 1, 1, 1, 0, 0, 0);
            pipe_barrier(PIPE_V);
            vsel(
                (__ubuf__ half*)tmpPtr + 2 * tmpGapEles, (__ubuf__ half*)tmpPtr + 2 * tmpGapEles,
        ...
        pipe_barrier(PIPE_V);
        vconv_f162s32a(
            (__ubuf__ int32_t*)dstPtr + numLoop * elemPerRpt, (__ubuf__ half*)tmpPtr + 2 * tmpGapEles, 2, 1, 1, 8, 4);
        set_mask_norm();
        set_vector_mask(-1, -1);
    }
}
...
template <typename TileDataOut, typename TileDataIn, typename TileDataTmp>
__tf__ PTO_INTERNAL void TColArgMin32(
    typename TileDataOut::TileDType __out__ dst, typename TileDataIn::TileDType __in__ src,
    typename TileDataTmp::TileDType __in__ tmp, unsigned srcValidRow, unsigned srcValidCol)
{
    using TOUT = typename TileDataOut::DType;
    ...
    if (numLoop > 0) {
        for (uint16_t j = 0; j < numLoop; j++) {
            ...
            for (uint16_t i = 1; i < srcValidRow; i++) {
                ...
                vcmp_le(
                    (__ubuf__ float*)tmpPtr + tmpGapEles, (__ubuf__ float*)srcPtr + i * srcRowStride + j * elemPerRpt,
                    1, 1, 1, 1, 0, 0, 0);
                pipe_barrier(PIPE_V);
                vsel(
                    (__ubuf__ float*)dstPtr + j * elemPerRpt, (__ubuf__ float*)dstPtr + j * elemPerRpt,
                    (__ubuf__ float*)tmpPtr, 1, 1, 1, 1, 0, 0, 0, 0);
                vmin(
                    (__ubuf__ float*)tmpPtr + tmpGapEles, (__ubuf__ float*)tmpPtr + tmpGapEles,
    ...
    if (remainAfterLoop > 0) {
        ...
        for (uint16_t i = 1; i < srcValidRow; i++) {
            ...
            vadds((__ubuf__ int32_t*)tmpPtr, (__ubuf__ int32_t*)tmpPtr, 1, 1, 1, 1, 0, 0);
            vcmp_le(
                (__ubuf__ float*)tmpPtr + tmpGapEles, (__ubuf__ float*)srcPtr + i * srcRowStride + numLoop * elemPerRpt,
                1, 1, 1, 1, 0, 0, 0);
            pipe_barrier(PIPE_V);
            vsel(
                (__ubuf__ float*)dstPtr + numLoop * elemPerRpt, (__ubuf__ float*)dstPtr + numLoop * elemPerRpt,
                (__ubuf__ float*)tmpPtr, 1, 1, 1, 1, 0, 0, 0, 0);
            vmin(
                (__ubuf__ float*)tmpPtr + tmpGapEles, (__ubuf__ float*)tmpPtr + tmpGapEles,
...
PTO_INTERNAL void TCOLARGMIN_IMPL(TileDataOut& dst, TileDataIn& src, TileDataTmp& tmp)
{
    ...
    if constexpr (sizeof(typename TileDataIn::DType) == 2) {
        TColArgMin16<TileDataOut, TileDataIn, TileDataTmp>(
            dst.data(), src.data(), tmp.data(), srcValidRow, srcValidCol);
    } else if constexpr (sizeof(typename TileDataIn::DType) == 4) {
        TColArgMin32<TileDataOut, TileDataIn, TileDataTmp>(
            dst.data(), src.data(), tmp.data(), srcValidRow, srcValidCol);
    }
}
} // namespace pto
