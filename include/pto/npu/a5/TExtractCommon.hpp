...
template <typename DstTileData, typename SrcTileData>
__tf__ PTO_INTERNAL void TCopyNdToNzUbToMat(
    typename DstTileData::TileDType __out__ dst, typename SrcTileData::TileDType __in__ src, uint32_t validRows,
    uint32_t validCols, uint32_t srcRowOffset = 0, uint32_t srcColOffset = 0, uint32_t dstRowOffset = 0,
    uint32_t dstColOffset = 0)
{
    using T = typename SrcTileData::DType;
    static_assert(std::is_same_v<T, typename DstTileData::DType>, "UB-to-L1 ND-to-NZ requires matching data types.");
    static_assert(sizeof(T) <= 4, "UB-to-L1 ND-to-NZ supports b4/b8/b16/b32 data.");
    static_assert(
        SrcTileData::Loc == TileType::Vec && SrcTileData::isRowMajor && SrcTileData::SFractal == SLayout::NoneBox,
        "UB-to-L1 ND-to-NZ requires an ND Vec source.");
    static_assert(
        DstTileData::Loc == TileType::Mat && !DstTileData::isRowMajor && DstTileData::SFractal == SLayout::RowMajor &&
            DstTileData::SFractalSize == CUBE_BLOCK_SIZE && DstTileData::Compact == CompactMode::Null,
        "UB-to-L1 ND-to-NZ requires a non-compact NZ512 Mat destination.");
    constexpr bool IS_FP4 = std::is_same_v<T, float4_e2m1x2_t> || std::is_same_v<T, float4_e1m2x2_t>;
    constexpr uint32_t C0_SIZE = BLOCK_BYTE_SIZE / sizeof(T) * (IS_FP4 ? 2 : 1);
    constexpr uint32_t SRC_ROW_STRIDE_BLOCKS = SrcTileData::RowStride / C0_SIZE;
    constexpr uint32_t DST_COL_STRIDE_BLOCKS = DstTileData::Rows;
    static_assert(SrcTileData::RowStride % C0_SIZE == 0, "ND source row stride must be 32-byte aligned.");
    static_assert(
        SRC_ROW_STRIDE_BLOCKS > 0 && SRC_ROW_STRIDE_BLOCKS <= 65536 && DST_COL_STRIDE_BLOCKS <= 65536,
        "UB-to-L1 ND-to-NZ DMA gaps must fit in 16 bits.");
    PTO_ASSERT(
        validCols % C0_SIZE == 0 && srcColOffset % C0_SIZE == 0 && dstColOffset % C0_SIZE == 0,
        "UB-to-L1 ND-to-NZ columns and column offsets must be 32-byte aligned.");
    PTO_ASSERT(
        srcRowOffset + validRows <= SrcTileData::Rows && srcColOffset + validCols <= SrcTileData::Cols,
        "UB-to-L1 ND-to-NZ source window exceeds storage.");
    PTO_ASSERT(
        dstRowOffset + validRows <= DstTileData::Rows && dstColOffset + validCols <= DstTileData::Cols,
        "UB-to-L1 ND-to-NZ destination window exceeds storage.");
    if (validRows == 0 || validCols == 0) {
        return;
    }
    __ubuf__ uint8_t* srcPtr = (__ubuf__ uint8_t*)__cce_get_tile_ptr(src) +
                               (srcRowOffset * SRC_ROW_STRIDE_BLOCKS + srcColOffset / C0_SIZE) * BLOCK_BYTE_SIZE;
    __cbuf__ uint8_t* dstPtr = (__cbuf__ uint8_t*)__cce_get_tile_ptr(dst) +
                               (dstColOffset / C0_SIZE * DST_COL_STRIDE_BLOCKS + dstRowOffset) * BLOCK_BYTE_SIZE;
    uint32_t colBlockCount = validCols / C0_SIZE;
    constexpr uint32_t MAX_BURST_COUNT = 4095;
    // Each burst moves one row fragment. Choose the shorter outer loop without UB scratch.
    if (colBlockCount > validRows) {
        for (uint32_t row = 0; row < validRows; ++row) {
            for (uint32_t colBlock = 0; colBlock < colBlockCount; colBlock += MAX_BURST_COUNT) {
                uint16_t burstCount = static_cast<uint16_t>(min(colBlockCount - colBlock, MAX_BURST_COUNT));
                copy_ubuf_to_cbuf(
                    dstPtr + (colBlock * DST_COL_STRIDE_BLOCKS + row) * BLOCK_BYTE_SIZE,
                    srcPtr + (row * SRC_ROW_STRIDE_BLOCKS + colBlock) * BLOCK_BYTE_SIZE, 0, burstCount, 1, 0,
                    DST_COL_STRIDE_BLOCKS - 1);
            }
        }
    } else {
        for (uint32_t colBlock = 0; colBlock < colBlockCount; ++colBlock) {
            for (uint32_t row = 0; row < validRows; row += MAX_BURST_COUNT) {
                uint16_t burstCount = static_cast<uint16_t>(min(validRows - row, MAX_BURST_COUNT));
                copy_ubuf_to_cbuf(
                    dstPtr + (colBlock * DST_COL_STRIDE_BLOCKS + row) * BLOCK_BYTE_SIZE,
                    srcPtr + (row * SRC_ROW_STRIDE_BLOCKS + colBlock) * BLOCK_BYTE_SIZE, 0, burstCount, 1,
                    SRC_ROW_STRIDE_BLOCKS - 1, 0);
            }
        }
    }
}
