    ...
    __VEC_SCOPE__
    {
        ...
        if (version == VFIMPL_2D_NO_POST_UPDATE) {
            for (uint16_t i = 0; i < (uint16_t)rows; ++i) {
                ...
                if constexpr (std::is_same_v<TIN, int16_t> && std::is_same_v<TOUT, int32_t>) {
                    RegTensor<TIN> vregdst_trunc;
                    vcvt(vregdst_trunc, vregdst, pregdst, RS_ENABLE, PART_EVEN);
                    vcvt(vregdst_trunc, vregdst, pregdst, RS_ENABLE, PART_ODD);
