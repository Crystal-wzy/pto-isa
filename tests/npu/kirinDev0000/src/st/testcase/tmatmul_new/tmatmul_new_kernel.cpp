    ...
    __gm__ OutType* out, __gm__ AType* src0, __gm__ BType* src1, __gm__ BiasType* src2)
{
    ...
    using BiasTileData = Tile<TileType::Bias, OutType, 1, BASEN, BLayout::RowMajor, 1, N>;

    using RightTile = TileRight<BType, BASEK, BASEN, BASEK, N>;
    using AccTile = Tile<TileType::Mat, OutType, BASEM, BASEN, BLayout::ColMajor, M, N, SLayout::RowMajor, 1024>;
    using VecTile = Tile<TileType::Vec, OutType, BASEM, BASEN, BLayout::ColMajor, M, N, SLayout::RowMajor, 1024>;
    ...
    for (int i = 0; i < iter; i++) {
        ...
        if (i == 0) {
            if constexpr (isBias) {
                TMATMUL_BIAS(cMatTile, aMatTile, bTile, biasTile);
            } else {
                TMATMUL(cMatTile, aMatTile, bTile);
            }
        } else {
            TMATMUL_ACC(cMatTile, cMatTile, aMatTile, bTile);
        }
