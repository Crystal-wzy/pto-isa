...
template <Nd2NzOperation Operation>
void testUbToL1Nd2Nz()
{
    constexpr int SRC_ROWS = 48;
    constexpr int SRC_COLS = 64;
    constexpr int DST_ROWS = 32;
    constexpr int DST_COLS = 48;
    constexpr int VALID_ROWS = 17;
    constexpr int VALID_COLS = 24;
    constexpr int INDEX_ROW = 3;
    constexpr int INDEX_COL = 8;
    constexpr int C0_SIZE = 8;
    using SrcTile = Tile<TileType::Vec, float, SRC_ROWS, SRC_COLS, BLayout::RowMajor, DYNAMIC, DYNAMIC>;
    using DstTile =
        Tile<TileType::Mat, float, DST_ROWS, DST_COLS, BLayout::ColMajor, DYNAMIC, DYNAMIC, SLayout::RowMajor>;
    SrcTile src(
        Operation == Nd2NzOperation::EXTRACT ? SRC_ROWS : VALID_ROWS,
        Operation == Nd2NzOperation::EXTRACT ? SRC_COLS : VALID_COLS);
    DstTile dst(
        Operation == Nd2NzOperation::INSERT ? DST_ROWS : VALID_ROWS,
        Operation == Nd2NzOperation::INSERT ? DST_COLS : VALID_COLS);
    TASSIGN(src, 0);
    TASSIGN(dst, 0);
    for (int i = 0; i < SRC_ROWS * SRC_COLS; ++i) {
        src.data()[i] = static_cast<float>(i);
    }
    std::fill(dst.data(), dst.data() + DST_ROWS * DST_COLS, -1.0f);
    std::vector<float> expected(DST_ROWS * DST_COLS, -1.0f);
    for (int row = 0; row < VALID_ROWS; ++row) {
        for (int col = 0; col < VALID_COLS; ++col) {
            ...
            const int srcCol = col + (Operation == Nd2NzOperation::EXTRACT ? INDEX_COL : 0);
            const int dstRow = row + (Operation == Nd2NzOperation::INSERT ? INDEX_ROW : 0);
            const int dstCol = col + (Operation == Nd2NzOperation::INSERT ? INDEX_COL : 0);
            const int offset =
                ((dstCol / C0_SIZE * (DST_ROWS / 16) + dstRow / 16) * 16 + dstRow % 16) * C0_SIZE + dstCol % C0_SIZE;
            expected[offset] = static_cast<float>(srcRow * SRC_COLS + srcCol);
        }
    }
    if constexpr (Operation == Nd2NzOperation::EXTRACT) {
        TEXTRACT(dst, src, INDEX_ROW, INDEX_COL);
    } else if constexpr (Operation == Nd2NzOperation::INSERT) {
        TINSERT(dst, src, INDEX_ROW, INDEX_COL);
    } else {
        TMOV(dst, src);
    }
    EXPECT_TRUE(std::equal(expected.begin(), expected.end(), dst.data()));
}
