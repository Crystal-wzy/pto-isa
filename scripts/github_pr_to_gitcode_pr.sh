...
# Apply only this PR's commits (base..head) onto cann:master.
...
if ! git cherry-pick "${cherry_range}"; then
  ...
  die "cherry-pick onto ${GITCODE_BASE_BRANCH} failed; the PR may depend on changes that exist on the GitHub side but not in cann:master"
fi
